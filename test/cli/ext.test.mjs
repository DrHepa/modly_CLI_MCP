...
test('help advertises ext stage github as preflight only, ext apply as live-target, and ext setup-status as target-scoped', () => {
  ...
  assert.match(globalHelp, /ext <subcommand>\s+reload \| errors \| stage github \| apply \| setup \| setup-status \| repair/u);
  assert.match(globalHelp, /stage github\s+Stage\/preflight only from GitHub; prepares and inspects, does NOT install or apply live/u);
  assert.match(globalHelp, /apply\s+Installs an already prepared stage onto the live target; requires explicit --extensions-dir, may trigger live-target setup if the stage requires it, and may finish as applied_degraded when that setup fails/u);
  assert.match(globalHelp, /setup\s+Runs ONLY an explicit contract/u);
  assert.match(globalHelp, /some extensions require explicit inputs such as gpu_sm/u);
  assert.match(globalHelp, /setup-status\s+Reads ONLY the installed target journal from the last observable setup; --wait\/--follow observe locally, --timeout-ms does not kill the process/u);
  assert.match(globalHelp, /repair\s+Reapplies an already prepared stage; may trigger live-target setup if the stage requires it, accepts the same relevant setup flags as apply, and by default no longer creates a backup when the extension exists/u);
  ...
  assert.match(extHelp, /installs an already prepared stage onto the live target/u);
  assert.match(extHelp, /may trigger live-target setup if the stage contract requires it/u);
  assert.match(extHelp, /accepts --python-exe, --allow-third-party, and --setup-payload-json to forward them to live-target setup/u);
  assert.match(extHelp, /CLI-only setup on an already prepared stage/u);
  assert.match(extHelp, /setup-status\s+reads ONLY the installed target's live-target journal/u);
  assert.match(extHelp, /requires explicit --extensions-dir and \(--manifest-id or --stage-path only to resolve manifest\.id\)/u);
  assert.match(extHelp, /does NOT reattach, does NOT cancel, and is NOT a general job manager/u);
  assert.match(extHelp, /--wait waits locally until an observable terminal journal state/u);
  assert.match(extHelp, /--follow follows the logPath of the most recent observable run locally/u);
  assert.match(extHelp, /--timeout-ms only cuts off CLI wait\/follow; it does NOT kill or cancel the underlying setup/u);
  assert.match(extHelp, /no background manager, reattach, or general-purpose resume/u);
  assert.match(extHelp, /cataloged and limited support; does not promise universal compatibility/u);
  assert.match(extHelp, /PIP_\*-based network resilience may provide partial benefit or none if setup\.py ignores those variables or performs its own downloads/u);
  assert.match(extHelp, /python_exe and ext_dir are auto-injected by the CLI and the stage/u);
  assert.match(extHelp, /requires explicit consent because it executes third-party code/u);
  assert.match(extHelp, /repair as CLI-only reapply over an already prepared stage/u);
  assert.match(extHelp, /ext repair accepts --python-exe, --allow-third-party, and --setup-payload-json to forward them to live-target setup when the stage requires it/u);
  assert.match(extHelp, /may trigger live-target setup if the stage requires it/u);
  assert.match(extHelp, /the result may remain observable as applied_degraded/u);
  assert.match(extHelp, /some extensions require explicit payload inputs, for example gpu_sm/u);
  assert.match(extHelp, /by default no longer creates a backup when the target exists/u);
  assert.match(extHelp, /seam failure vs the extension's own failure/u);
  assert.match(extHelp, /Linux ARM64/u);
  assert.match(extHelp, /Does NOT perform GitHub fetch, install, build, or general health-fix/u);
  ...
  assert.match(extHelp, /Does not expose a stable MCP capability/u);
  assert.match(extHelp, /Does NOT install or apply live/u);
});
